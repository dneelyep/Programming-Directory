...
class PreferencesDaerotDialog(gtk.Dialog):
    __gtype_name__ = "PreferencesDaerotDialog"
    prefernces = {}

    def __init__(self):
        """__init__ - This function is typically not called directly.
        Creation of a PreferencesDaerotDialog requires redeading the associated ui
        file and parsing the ui definition extrenally,
        and then calling PreferencesDaerotDialog.finish_initializing().

        Use the convenience function NewPreferencesDaerotDialog to create
    ...
    def get_preferences(self):
        ...
        preferences for daerot. Creates a couchdb record if
        necessary.
        """
...
def NewPreferencesDaerotDialog():
    """NewPreferencesDaerotDialog - returns a fully instantiated
    PreferencesDaerotDialog object. Use this function rather than
    creating a PreferencesDaerotDialog instance directly.
    """

    #look for the ui file that describes the ui
    ui_filename = os.path.join(getdatapath(), 'ui', 'PreferencesDaerotDialog.ui')
    if not os.path.exists(ui_filename):
        ui_filename = None

    builder = gtk.Builder()
    builder.add_from_file(ui_filename)
    dialog = builder.get_object("preferences_daerot_dialog")
    dialog.finish_initializing(builder)
    return dialog

if __name__ == "__main__":
    dialog = NewPreferencesDaerotDialog()
    dialog.show()
    gtk.main()
